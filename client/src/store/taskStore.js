...
export const useTaskStore = create((set, get) => ({
  ...
  toggleTask: (id) => {
    ...
    // Optimistic
    set((s) => ({
      tasks: s.tasks.map(t => t.id === id ? { ...t, completed: completed ? 1 : 0, status: completed ? 'done' : 'todo' } : t),
    }))
