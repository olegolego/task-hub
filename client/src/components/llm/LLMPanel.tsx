...
export default function LLMPanel() {
  ...
  function handleInputChange(e) {
    ...
    if (match) {
      ...
      const ctxMatches = MENTION_CONTEXTS.filter(
        (c) => c.key.startsWith(query) || c.label.toLowerCase().startsWith(query),
  ...
  function handleKeyDown(e) {
    if (mention.active) {
      if (e.key === 'ArrowDown') {
        ...
        setMention((m) => ({
          ...m,
          selectedIndex: Math.min(m.selectedIndex + 1, m.filtered.length - 1),
        }))
        return
      }
      if (e.key === 'ArrowUp') {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        >
          {!activeChatId && <EmptyState status={llmStatus} onNew={newChat} />}
          {loadingHistory && (
            <div
              style={{
                color: 'var(--text-secondary)',
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
            <textarea
              ...
              placeholder={
                activeChatId
                  ? 'Ask anything… type @ to reference data sources'
                  : 'Start a new chat above, then ask something…'
              }
              rows={2}
          ...
          >
            ...
            <span style={{ fontSize: 10, color: 'var(--text-secondary)' }}>
              Include company files in context
            </span>
          </label>
        </form>
      </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderLeft:
                    i === selectedIndex ? '2px solid var(--accent)' : '2px solid transparent',
        ...
        <>
          <div
            style={{
              ...
              letterSpacing: 0.8,
              opacity: 0.6,
              borderTop: ctxItems.length > 0 ? '1px solid rgba(255,255,255,0.06)' : 'none',
            }}
          >
            Files
          </div>
