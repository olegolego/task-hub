          ...
          >
            ...
            <span
              style={{
                ...
                fontWeight: 600,
                color:
                  voteCount > 0 ? '#06d6a0' : voteCount < 0 ? '#ef476f' : 'var(--text-secondary)',
                minWidth: 14,
                textAlign: 'center',
              }}
            >
          ...
          >
            {Object.entries(STATUS_COLORS).map(([status, color]) => (
              <button
                key={status}
                onClick={() => {
                  changeIdeaStatus(idea.id, status)
                  setShowMenu(false)
                }}
                style={{
                  display: 'flex',
                ...
                onMouseLeave={(e) =>
                  (e.currentTarget.style.background =
                    idea.status === status ? 'var(--hover)' : 'transparent')
                }
              >
                <span
                  style={{
      ...
      {expanded && (
        ...
        >
              ...
              <div key={c.id} style={{ display: 'flex', gap: 7, marginBottom: 8 }}>
                <span
                  style={{
                    ...
                    height: 20,
                    borderRadius: '50%',
                    background: c.avatarColor || '#4361ee',
                    flexShrink: 0,
                    display: 'flex',
                    alignItems: 'center',
          ...
          <form
            ...
            style={{ display: 'flex', gap: 6, padding: '4px 10px 8px' }}
          >
            <input
              ref={commentRef}
              value={commentInput}
            ...
            <button
              ...
              style={{
                background: commentInput.trim() ? 'var(--accent)' : 'var(--surface)',
                border: 'none',
                borderRadius: 6,
                color: commentInput.trim() ? '#1a1a2e' : 'var(--text-secondary)',
                fontSize: 11,
                fontWeight: 600,
                padding: '4px 10px',
