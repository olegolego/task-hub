              ...
              {otherUsers.map((u) => (
                <label
                  ...
                  style={{
                    ...
                    padding: '4px 6px',
                    borderRadius: 6,
                    background: attendeeIds.includes(u.id)
...
function MeetingPopover({ meeting, anchorRect, onClose }: MeetingPopoverProps) {
  ...
  return (
    <>
      ...
      <div style={style}>
        <div
          style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.3 }}
        >
          {meeting.title}
        </div>
          ...
          {meeting.attendees?.map((a) => (
            <div key={a.userId} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span
                style={{
                  fontSize: 10,
                  color: statusColors[a.status] || 'var(--text-secondary)',
                  fontWeight: 600,
                }}
              >
                {a.status}
...
function WeekGrid({ weekStart, meetings, onMeetingClick }: WeekGridProps) {
  ...
  function getMeetingsForDay(dayDate: Date) {
    ...
    return meetings.filter(
      (m) => m.startTime <= dayEnd.getTime() && m.endTime >= dayStart.getTime(),
    )
  }
  ...
  function getBlockStyle(meeting: any) {
    ...
    const startHour =
      new Date(meeting.startTime).getHours() + new Date(meeting.startTime).getMinutes() / 60
    const endHour =
      new Date(meeting.endTime).getHours() + new Date(meeting.endTime).getMinutes() / 60
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      {/* Day header row */}
      <div
        style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.08)', flexShrink: 0 }}
      >
          ...
          <div style={{ width: 44, flexShrink: 0, position: 'relative' }}>
            {hours.map((h) => (
              <div
                ...
                style={{
                  position: 'absolute',
                  top: (h - HOUR_START) * HOUR_HEIGHT - 7,
                  right: 6,
                  fontSize: 10,
                  color: 'var(--text-secondary)',
                  userSelect: 'none',
    ...
    >
      ...
      {pending.map((m) => (
        <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button
            onClick={() => respondToMeeting(m.id, 'accepted')}
            style={{
              fontSize: 10,
              padding: '2px 8px',
              borderRadius: 4,
          ...
          <button
            onClick={() => respondToMeeting(m.id, 'declined')}
            style={{
              fontSize: 10,
              padding: '2px 8px',
              borderRadius: 4,
...
export default function CalendarPanel() {
    ...
    function check() {
      ...
      for (const m of meetings) {
        ...
        if (diff <= 5 * 60 * 1000 && diff > -60 * 1000) {
          ...
          setToasts((prev) => [
            ...prev,
            { id, text: `"${m.title}" starts in ${Math.max(0, Math.round(diff / 60000))} min` },
          ])
        }
      }
    }
  ...
  return (
    ...
    >
      ...
      >
        <button onClick={() => setWeekStart((w) => addDays(w, -7))} style={navBtnStyle}>
          {'< Prev'}
        </button>
        <button onClick={goToToday} style={navBtnStyle}>
          Today
        </button>
        <button onClick={() => setWeekStart((w) => addDays(w, 7))} style={navBtnStyle}>
          {'Next >'}
        ...
        <span
          style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 600, marginLeft: 4 }}
        >
          {weekLabel}
        </span>
        <div style={{ flex: 1 }} />
