...
export default function FilesPanel() {
  ...
  const visible = activeFolder === 'All' ? files : files.filter(f => (f.folder || 'General') === activeFolder)

  async function handleUpload() {
    setUploading(true)
    try { await ipc.uploadCompanyFile(activeFolder === 'All' ? 'General' : activeFolder) }
  ...
  async function handleDownload(file) {
    ...
    try { await ipc.downloadCompanyFile({ fileId: file.id, fileName: file.name }) }
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ padding: '8px 10px', background: 'var(--surface)', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: 0.5, flex: 1 }}>
          Company Files
        </span>
        <button
          onClick={() => { setShowNewFolder(s => !s); setNewFolderName('') }}
          style={{ background: 'var(--hover)', border: 'none', borderRadius: 4, color: 'var(--text-secondary)', fontSize: 11, fontWeight: 600, padding: '3px 10px', cursor: 'pointer' }}
        >
          + Folder
        </button>
        <button
          ...
          disabled={uploading}
          style={{ background: 'var(--accent)', border: 'none', borderRadius: 4, color: '#1a1a2e', fontSize: 11, fontWeight: 600, padding: '3px 10px', cursor: uploading ? 'default' : 'pointer' }}
        >
      ...
      {showNewFolder && (
        <form onSubmit={handleNewFolder} style={{ padding: '6px 10px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 6 }}>
          <input
            value={newFolderName}
            onChange={e => setNewFolderName(e.target.value)}
            placeholder="Folder name…"
            autoFocus
            style={{ flex: 1, background: 'var(--bg)', border: '1px solid var(--accent)', borderRadius: 4, color: 'var(--text-primary)', fontSize: 12, padding: '3px 8px', outline: 'none', fontFamily: 'inherit' }}
          />
          <button type="submit" style={{ background: 'var(--accent)', border: 'none', borderRadius: 4, color: '#1a1a2e', fontSize: 11, fontWeight: 600, padding: '3px 8px', cursor: 'pointer' }}>Create</button>
          <button type="button" onClick={() => setShowNewFolder(false)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-secondary)', fontSize: 11 }}>Cancel</button>
        </form>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '6px 0' }}>
        ...
        {!loading && visible.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, padding: 32, color: 'var(--text-secondary)' }}>
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ opacity: 0.4 }}>
              <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" />
            </svg>
            <span style={{ fontSize: 12, opacity: 0.6, textAlign: 'center' }}>No files yet.<br />Upload one above.</span>
          </div>
        )}
        {visible.map(file => (
          <FileRow
      ...
      <div style={{ padding: '8px 12px', background: 'var(--surface)', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontSize: 10, color: 'var(--text-secondary)', lineHeight: 1.4 }}>
          Files stored at <code style={{ fontSize: 9, background: 'rgba(255,255,255,0.06)', padding: '1px 4px', borderRadius: 3 }}>~/.taskmanager-server/company-files/</code> — ready for local LLM indexing
        </span>
      </div>
    </div>
  )
