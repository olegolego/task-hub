...
export default function MessagesPanel() {
  const { threads, activeThreadUserId, unreadCounts, setActiveThread, sendMessage } =
    useMessageStore()
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            color: sending ? 'var(--text-secondary)' : 'var(--text-secondary)',
          ...
          onMouseEnter={(e) => {
            if (!sending) e.currentTarget.style.color = 'var(--accent)'
          }}
...
function FileMessage({ m, isMine: _isMine, myUserId: _myUserId }: FileMessageProps) {
  ...
  async function download() {
    ...
    try {
      ...
      const result = await ipc.downloadFileDM({
        fileId: m.fileId,
        fileName: m.fileName,
        encFileKey: m.encFileKey,
        fileKeyNonce: m.fileKeyNonce,
        fromUserId,
      })
